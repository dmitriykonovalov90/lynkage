...
salegroup_id = 101
organization_id = 1
partner_uuid = 'corebo00000000000nc91g82t6iheud0'
object_type = 'user'
...
def test_partners():
    url = 'https://api.uat.navigator.lynkage.ru/users/partners'
    ...
    assert response.status_code == 200, "Ошибка"
    requestdict = json.loads(response.content)
    assert requestdict['data'][0]['name'] == 'Атента_Гарантия+ РТ'
...
def test_content_work_time_graph_tele2():
    ...
    assert response.status_code == 200, "Ошибка"
    requestdict = json.loads(response.content)
    assert requestdict['data']['graphic']['values_scale']['name'] == 'Рабочее время'
...
def test_content_efficiency_graph_tele2():
    ...
    assert response.status_code == 200, "Ошибка"
    requestdict = json.loads(response.content)
    assert requestdict['data']['graphic']['values_scale']['name'] == 'Эффективность'


def test_content_widget_sales():
    url = 'https://node.uat.navigator.lynkage.ru/widgets/content'
    first_period = str(first_day_of_current_period)
    end_period = str(last_day_of_current_month)
    ...
    body = {
        "data_level": "operator",
        "user_id": id_users(),
        "widget_acronim": "widget_sales_tele2",
        "parameters":
            {
                "arm": "operator",
                "object_id": id_users(),
    ...
    assert response.status_code == 200, "Ошибка"
    requestdict = json.loads(response.content)
    assert requestdict['data']['efficiency']['acronim'] == 'efficiency'
...
#         "data_level": "rgp",
#         "user_id": id_users(),
#         "widget_acronim": "widget_revenue_forecast_tele2",
#         "parameters":
#             {
#              "arm": "brigadir",
#              "object_id": salegroup_id,
